...
const builtIns = require('./../core/built-in-functions').__fns__;
...
class NameResolver {
  ...
  find(varName) {
    ...
    return this.inputData.some(x => x.name === varName) || this.decisions.some(x => x.name === varName) || !!builtIns[varName]
...
NameResolver.MODE_CHILD_PATH_EXP = 'CHILD_PATH_EXPRESSION';
